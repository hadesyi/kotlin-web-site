...
(같은 패키지나 같은 이름 또는 같은 @JvmName 애노테이션을 사용해서) 생성할 자바 클래스 이름이 같은 파일이 여러개인 경우, 보통 에러가 발생한다.
하지만, 컴파일러는 지정한 이름을 갖고 그 이름을 갖는 파일에 선언한 모든 것을 포함하면 단일 자바 파사드 클래스를 만드는 기능을 제공한다.
그런 파사드 생성을 활성화하려면, 모든 파일에 @JvmMultifileClass 애노테이션을 사용하면 된다.
...
코틀린 프로퍼티를 자바의 필드로 노출하길 원한다면, `@JvmField` 애노테이션을 프로퍼티에 적용해야 한다.
...
필드의 가시성은 `lateinit` 프로퍼티 setter와 같은 가시성을 갖는다.
...
코틀린은 또한 네임드 오브젝트나 컴페니언 오브젝트에 정의한 함수에 `@JvmStatic`을 붙이면 정적 메서드를 생성한다.
다음 예를 보자:
...
생성자와 정적 메서드에도 이 애노테이션 적용이 가능하다. 인터페이스에 정의한 메서드를 포함해서 추상 메서드에는 사용할 수 없다.
...
앞서 말했듯, 코틀린에는 체크드 익셉션이 없다.
따라서, 보통, 코틀린 함수의 자바 시그너처는 익셉션을 선언하지 않는다.
다음과 같은 코틀린 함수를 가졌다고 해 보자:
...
코틀린이 [declaration-site variance](generics.html#declaration-site-variance)을 사용하면,
...
왜냐면 자바에서 `Box` 클래스는 `T` 파라미터에 대해 *invariant*라서 `Box<Derived>`가 `Box<Base>`의 하위타입이 아니기 때문이다.
...
여기서 use-site variance를 이용해서 declaration-site variance를 흉내내기 위해
자바의 *와일드카드 타입*(`? extends Base`)을 사용했다. 왜냐면 자바로는 이렇게밖에 못하기 때문이다(because it is all Java has).

코틀린 API를 자바에서 쓸 수 있게 하려면, `Box<Super>`가 *as a parameter*일 때 `Box`를 covariant하게 정의한 `Box<? extends Super>`로
(또는 contravariant하게 정의한 `Foo<? super Bar>`) 만든다.
그것이 리턴 값을 가지면, 와일드카드를 생성하지 않는다. 그렇지 않을 경우 자바 클라이언트가 그것을 처리해야 하기 때문이다(그리고 이는 보통의 자바 코딩 방식과 반대된다).
따라서, 실제로 위 예의 함수를 다음과 같이 번역한다:
...
노트: 인지 타입이 final이면, 보통 와일드카드를 생성할 지점이 없기 때문에, `Box<String>`는 인지 위치에 상관없이 항상 `Box<String>`이다.

기본적으로 와일드카다를 생성할 수 없는 곳에서 와일드카드가 필요하면, `@JvmWildcard` 애노테이션을 사용할 수 있다:
