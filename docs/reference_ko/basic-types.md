...
코틀린은 모든 게 객체라서 변수에 대해 멤버 함수나 프로퍼티를 사용할 수 있다. 최적화를 위해 일부 타입을 제공하고 있지만 사용자에게는 일반 클래스와 같다. 이 절에서는 이 타입 중 숫자, 문자, 불린, 배열에 대해 설명한다.

## 숫자

코틀린은 자바와 유사한 방법으로 숫자를 다루지만 완전히 동일한 것은 아니다. 예를 들어, 숫자에 대해 큰 범위로의 자동 변환이 없고, 리터럴은 경우에 따라 약간 다르다.
...
자바 플랫폼에서 물리적으로 JVM 기본 타입으로 숫자를 보관한다.
nullable 숫자 참조(예, `Int?`)가 필요하거나 지네릭과 연관이 있다면 박싱 타입을 사용한다.

숫자를 박싱하면 identity(참조 동일)는 유지되지 않는 것에 유의해야 한다.
...
표현이 다르기 때문에, 작은 범위를 갖는 타입은 큰 범위를 갖는 타입의 하위 타입이 아니다.
만약 하위 타입이 된다면, 다음과 같은 상황에서 문제가 된다:
...
따라서, identity뿐만 아니라, 모든 곳에서 equality까지 읽게 될 수 있다.
...
자동 타입 변환이 없어도 거의 문제가 되진 않는다. 문맥에서 타입 추론이 가능하고, 수치 연산자를 각 타입 변환에 맞게 오버로딩했기 때문이다.
...
코틀린은 숫자에 대한 표준 수치 연산을 지원한다. 이 연산자는 각 타입의 멤버로 정의되어 있다(컴파일러가 해당하는 명령어로 최적화한다).
...
특수 문자를 역슬래시를 이용해서 표시한다.
지원하는 이스케이프 시퀀스는 다음과 같다: `\t`, `\b`, `\n`, `\r`, `\'`, `\"`, `\\` 그리고 `\$`.
다른 문자를 입력하려면, 유니코드 이스케피으 시퀀스 구문을 사용한다: `'\uFF00'`.
...
숫자처럼, nullable 참조가 필요하면 문자를 박싱한다. 박싱 연산은 identity를 유지하지 않는다.

## 부울(Boolean)

`Boolean` 타입은 부울을 표현하고 두 값을 갖는다: *true*{: .keyword }와 *false*{: .keyword }.
...
`arrayOfNulls()` 라이브러리 함수를 사용하면 null을 값으로 갖는 지정한 크기의 배열을 생성할 수도 있다.
...
주의: 자바와 달리 코틀린의 배열은 부공변성(invarint)이다. 이는 `Array<String>`을 `Array<Any>`에 할당할 수 없음을 의미하는데,
이는 가능한 런타임 실패를 방지한다(하지만 `Array<out Any>`는 사용할 수 있는데 이에 대한 내용은 [Type Projections](generics.html#type-projections)을 참고한다).
...
`String` 타입으로 문자열을 표현한다. 문자열을 불변(immutable)이다.
문자열의 요소는 문자로서 인덱스 연산을 이용해서 접근할 수 있다: `s[i]`
*for*{: .keyword }-루프를 통해 문자열의 각 문자를 접근할 수 있다:
...
템플릿 표현식은 달라 기호($)로 시작하고 단순 이름을 포함하거나,
