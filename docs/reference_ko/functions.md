...
코틀린은 파일에서 함수를 최상위 레벨로 선언할 수 있다. 자바, C#이나 스칼라와 같은 언어처럼 함수를 갖는 클래스를 만들 필요가 없다.
...
이 코드는 수학의 상수값인 코사인의 고정소수점을 계산한다. 이 코드는 1.0에서 시작해서 더 이상 값이 바뀌지 않을 때까지 Math.cos를 반복해서 호출한다. 결과는 0.7390851332151607이다. 이 코드는 전통적인 방식의 다음 코드와 같다.
