...
위 람다 식 구문에서 빠진 게 하나 있는데 그것은 바로 리턴 타입을 지정하는 방법이다.
...
임의 함수는 이름이 없다는 것을 제외하면 일반 함수 선언과 비슷하다.
...
임의 함수의 리턴 타입은 일반 함수와 동일하게 유추한다.
...
람다 식이나 임의 함수 (또는 [로컬 함수](functions.html#local-functions)) 그리고 [오브젝트 식](object-declarations.html#object-expressions)은
...
이를 사용하는 가장 중요한 한 가지 예가 [타입-안전 그루브-스타일 빌더](type-safe-builders.html)이다.
