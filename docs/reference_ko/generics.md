...
코틀린에는 복잡한 와이들 카드가 없다. 대신 선언-위치 가변(declaration-site variance)과 타입 프로젝션(type projection)의 두 가지를 제공한다.
...
이 메서드의 시그너처는 무엇인가? 직관적으로 생각하면 다음과 같이 작성할 수 있다.
...
*주의*: producer 객체를 사용한다면, 예를 들어 `List<? extends Foo>`, 이 객체에 대해 `add()`나 `set()`을 호출하는 것을 허용하지 않는다.
하지만 이것이 이 객체가 **불변(immutable)**인 것을 의미하는 것은 아니다. 예를 들어, 리스트의 모든 항목을 삭제하기 위해 `clear()`를 호출하는 것은 가능하다.
...
이 코드의 문제를 고치려면 `Source<? extends Object>` 타입 객체를 선언해야 하는데 이는 다소 의미가 없다. 왜냐면 수정 전에도 동일한 메서드를 호출할 수 있고
더 복잡한 타입으로 수정해도 값을 추가하지 않기 때문이다. 하지만 컴파일러는 이를 알지 못한다.
...
"전문 용어"로 클래스 `C`는 파라미터 `T`에 **공변(covariant)**한다 또는 `T`는 **공변(covariant)** 타입 파라미터라고 말한다.
...
단어 **in**과 **out**은 자명하므로(이미 꽤 오랜 시간 C#에서 성공적으로 사용하고 있다)
...
타입 파라미터 T를 *out*으로 선언하는 것은 매우 편리하고 사용 위치에서 하위타입 관련 문제가 없다.
좋다. 그런데 클래스가 실제로 `T`만 리턴하도록 제한이 있을 때 그것으로 못하는 건 무얼까?
Array가 좋은 예이다.
...
여기서 사용한 것을 **타입 프로젝션(type projection)**이라고 한다. 이 코드에서 `from`은 단순 배열이 아닌 **제한된(projected)** 배열이다. 오직 타입 파라미터 `T`를 리턴하는 메서드만 호출할 수 있다.
이 예의 경우 `get()`만 호출할 수 있다. 이것이 코틀린의 **사용-위치 가변(use-site variance)** 접근 방식이다. 자바의 `Array<? extends Object>`에 해당하지만 더 간단하다.
...
uppber bound를 지정하지 않을 경우 기본 upper bound는 `Any?`이다. 화살괄호 안에 오직 한 개의 upper bound만 지정할 수 있다.
